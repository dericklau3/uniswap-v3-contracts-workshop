...
library NFTDescriptor {
    ...
    struct DecimalStringParams {
        ...
        // 十进制字符串总长度
        uint8 bufferLength;
        // 有效数字的结束索引，后续复制会从后向前进行
        uint8 sigfigIndex;
        // 小数点索引；不需要小数点时为 0
        uint8 decimalIndex;
        // 极小或极大数值中前导/尾随 0 的起始索引
        uint8 zerosStartIndex;
        // 极小或极大数值中前导/尾随 0 的结束索引
        uint8 zerosEndIndex;
        // 十进制数是否小于 1
        bool isLessThanOne;
        // 字符串是否需要包含百分号
    ...
    // @notice 将平方根价格格式化为包含前 5 位有效数字的十进制字符串
    // @param sqrtRatioX96 Q64.96 平方根价格
    function fixedPointToDecimalString(
        ...
        if (priceBelow1) {
            // 10 ** 43 可为最小可能价格保留 5 位有效数字，并额外保留 1 位用于舍入
            value = FullMath.mulDiv(value, 10**44, 1 << 128);
        } else {
            // 保留 4 位小数精度，并额外保留 1 位用于舍入
    ...
    // @notice 将池费率格式化为十进制百分比字符串
    // @param fee 以百万分之一为单位的池费率
